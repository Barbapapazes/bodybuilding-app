<template lang="pug">
#show-training
  v-data-table(:headers="headers",  :items="getSelectedTraining[0].exercises", item-key="name",denses, :expanded.sync="expanded",  v-if="getSelectedTraining.length > 0").background
    template(v-slot:top)
    ...
    template(v-slot:item.data-table-expand="{ item }")
      v-btn(@click="expanded = [item]" v-if="!expanded.includes(item)", x-small, text)
        v-icon(small) {{svgPath.mdiChevronDown}}
      v-btn(@click="expanded = []" v-if="expanded.includes(item)", x-small, text)
